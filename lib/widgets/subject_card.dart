...
class SubjectCard extends StatelessWidget {
  ...
  int _calculateClassesNeeded(Subject subject, double threshold) {
    ...
    // Calculate classes needed: (attended + x) / (total + x) >= threshold/100
    // Solving: attended + x >= (threshold/100) * (total + x)
    // x >= (threshold/100 * total - attended) / (1 - threshold/100)
    
    final double targetRatio = threshold / 100;
    final double numerator = targetRatio * total - attended;
    final double denominator = 1 - targetRatio;
    
    if (denominator <= 0) return 0; // Edge case when threshold is 100%
    
    final int classesNeeded = (numerator / denominator).ceil();
